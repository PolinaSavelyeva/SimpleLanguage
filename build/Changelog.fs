...
let mkLinkReference (newVersion: SemVerInfo) (changelog: Changelog.Changelog) (gitHubRepoUrl: string) =
    if changelog.Entries |> List.isEmpty then
        ...
        let prevEntry =
            changelog.Entries
            |> List.skipWhile (fun entry -> entry.SemVer.PreRelease.IsSome || versionTuple entry.SemVer = versionTuple newVersion)
            |> List.tryHead
...
let updateChangelog changelogPath (changelog: Fake.Core.Changelog.Changelog) gitHubRepoUrl ctx =
    ...
    let newEntry =
        Changelog.ChangelogEntry.New(assemblyVersion.Value, nugetVersion.Value, Some System.DateTime.Today, description, unreleasedChanges @ prereleaseChanges, false)
    ...
    let newLinkReferenceTargets =
        ...
        | first :: rest when first |> String.startsWith "[Unreleased]:" -> linkReferenceForUnreleased :: linkReferenceForLatestEntry :: rest
